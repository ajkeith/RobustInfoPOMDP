...
tip = SimpleIPOMDP(0.65, 0.55, 0.51, 0.7)
...
# intialize solver
solver = RPBVISolver(beliefpoints = bs, max_iterations = 200)
...
for i in 1:1
    ...
    @showprogress 1 "Simulating nominal value..." for j = 1:nreps
        sv, sp = simulate(psim, tip, solip, buip)
        simvals[i][j] = sv
        simps[i][j] = sp
    end
    ...
    srand(rseed)
    @showprogress 1 "Simulating nominal value..." for j = 1:nreps
        sv, sp = simulate(psim, tip, solrip, burip)
        simvals[i+1][j] = sv
        simps[i+1][j] = sp
    end
    ...
    srand(rseed)
    @showprogress 1 "Simulating nominal value..." for j = 1:nreps
        sv, sp = simulate(psim, tip, soltip, butip)
        simvals[i+2][j] = sv
        simps[i+2][j] = sp
    end
